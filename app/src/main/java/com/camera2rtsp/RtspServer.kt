 ...
 *
 * Outros dispositivos na mesma rede podem assistir via:
 *   rtsp://<IP_DO_CELULAR>:8554/live
 *
 * Construtores de RtspServerCamera2:
 *   RtspServerCamera2(context, connectChecker, port)      -- modo background
 *   RtspServerCamera2(openGlView, connectChecker, port)   -- com OpenGlView
 *   RtspServerCamera2(textureView, connectChecker, port)  -- com TextureView
 *
 * Usamos TextureView para exibir o preview na tela enquanto serve o stream.
 */
class RtspServer(
    ...
    // Inicializa com preview na TextureView
    ...
    fun start() {
        ...
        if (vOk && aOk) {
            cam.startStream("")
            Log.i(tag, "Servidor RTSP ativo: rtsp://<IP>:$port/live")
        } else {
    ...
    override fun onAuthSuccess()                    { Log.i(tag, "RTSP auth ok") }

    // -- ClientListener -------------------------------------------------------
    override fun onClientConnected(client: ServerClient) {
        connectedClients++
        Log.i(tag, "Cliente RTSP conectado: ${client.getAddress()} total=$connectedClients")
    }
    override fun onClientDisconnected(client: ServerClient) {
        connectedClients = maxOf(0, connectedClients - 1)
        Log.i(tag, "Cliente RTSP desconectado total=$connectedClients")
    }
    override fun onClientNewBitrate(bitrate: Long, client: ServerClient) {
        Log.d(tag, "Cliente ${client.getAddress()} bitrate=${bitrate}bps")
    }
}
