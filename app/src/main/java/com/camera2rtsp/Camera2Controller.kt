...
class Camera2Controller {
    ...
    private fun isoToEv(iso: Int): Int {
        if (iso <= 0) return 0
        val ratio = iso.toDouble() / 100.0
        val ev = (ln(ratio) / ln(2.0)).toInt()
        return ev.coerceIn(-8, 8)
    }

    // Referencia: 1/60s = 16_666_667ns = EV 0
    private fun shutterNsToEv(ns: Long): Int {
        if (ns <= 0L) return 0
        val refNs = 16_666_667.0
        val ev = (ln(ns.toDouble() / refNs) / ln(2.0)).toInt()
        return ev.coerceIn(-8, 8)
    }
    ...
    fun discoverAllCameras(context: Context): List<Map<String, Any>> {
        ...
        return mgr.cameraIdList.mapNotNull { id ->
            try {
                ...
                val availCaps = chars.get(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES) ?: return@mapNotNull null

                if (availCaps.contains(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_DEPTH_OUTPUT) &&
                    !availCaps.contains(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_BACKWARD_COMPATIBLE))
            ...
            } catch (e: Exception) { Log.w(tag, "discoverAllCameras id=$id: ${e.message}"); null }
        }
    }
